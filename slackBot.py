...
def main():
    ...
    if email_content_lines:
        ...
        for line in range(len(email_content_lines)):
            ...
            if email_content_lines[line].isupper():
                # Make the title bold and add a news counter in front of it
                final_content += f"{news_counter}. *{email_content_lines[line]}*\n"
                news_counter += 1
            else:
        ...
        response = client.chat_postMessage(channel='#bot-testing', text="*HEADLINES*\n" + final_content.strip())
        if response['ok']:
